...
struct MapDataB
{
	...
	int	tileWidth;
	int	tileHeight;
	List<TileSetB*> tilesets;
	MapTypesB type = MapTypesB::MAPTYPEB_ORTHOGONAL;

	// L05: DONE 2: Add a list/array of layers to the map
	List<MapLayerB*> maplayers;
	List<ImageLayerB*> imagelayers;
};
...
class Blacksmith : public Module
{
public:
...
private:
	...
	void CreateColliders(ColDataB colliders);

	bool LoadLayer(pugi::xml_node& node, MapLayerB* layer);
	bool LoadAllLayers(pugi::xml_node mapNode);

	// L06: DONE 2
	TileSetB* GetTilesetFromTileId(int gid) const;

	// L06: DONE 6: Load a group of properties 
	bool LoadProperties(pugi::xml_node& node, PropertiesB& properties);
