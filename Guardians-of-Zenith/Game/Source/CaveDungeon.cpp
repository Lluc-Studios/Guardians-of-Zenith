...
bool CaveDungeon::Awake(pugi::xml_node& config)
{
...
void CaveDungeon::Draw()
{
    if(mapLoaded == false)
        return;

    ListItem<ImageLayerC*>* imageLayerItem;
   ...
   // app->render->DrawRectangle({ -1 * (app->render->camera.x / (int)app->win->GetScale()),-1 * (app->render->camera.y / (int)app->win->GetScale()),app->render->camera.w,app->render->camera.h }, 0, 255, 0, 50);
    ...
    ListItem<MapLayerC*>* mapLayerItem;
        ...
        if (mapLayerItem->data->properties.GetProperty("Draw") != NULL && mapLayerItem->data->properties.GetProperty("Draw")->value) {
            ...
            for (int x = 0; x < mapLayerItem->data->width; x++)
            {
                for (int y = 0; y < mapLayerItem->data->height; y++)
                {
                    ...
                    TileSetC* tileset = GetTilesetFromTileId(gid);
...
bool CaveDungeon::CreateWalkabilityMap(int& width, int& height, uchar** buffer) const
{
    bool ret = false;
    ListItem<MapLayerC*>* item;
    item = mapData.maplayers.start;

    for (item = mapData.maplayers.start; item != NULL; item = item->next)
    {
        MapLayerC* layer = item->data;
        ...
        for (int y = 0; y < mapData.height; ++y)
        {
            for (int x = 0; x < mapData.width; ++x)
            {
                ...
                */
                TileSetC* tileset = (tileId > -1) ? GetTilesetFromTileId(tileId) : NULL;

                if (tileset != NULL)
                {
                    //map[i] = (tileId - tileset->firstgid) > 0 ? 0 : 1;
                    if (mapData.type == MapTypesC::MAPTYPEC_ISOMETRIC && tileId == 25) map[i] = 1;
                    else if (mapData.type == MapTypesC::MAPTYPEC_ORTHOGONAL && tileId == 119) map[i] = 1;
                    else map[i] = 0;
                }
                else {
...
iPoint CaveDungeon::MapToWorld(int x, int y) const
{
    iPoint ret;
...
iPoint CaveDungeon::WorldToMap(int x, int y)
{
    ...
    if (mapData.type == MAPTYPEC_ORTHOGONAL)
    {
        ret.x = x / mapData.tileWidth;
        ret.y = y / mapData.tileHeight;
    }
    else if (mapData.type == MAPTYPEC_ISOMETRIC)
    {
...
SDL_Rect TileSetC::GetTileRect(int gid) const
{
    SDL_Rect rect = { 0 };
...
TileSetC* CaveDungeon::GetTilesetFromTileId(int gid) const
{
    ListItem<TileSetC*>* item = mapData.tilesets.start;
    TileSetC* set = NULL;
...
bool CaveDungeon::LoadMap(pugi::xml_node mapFile)
{
    bool ret = true;
...
bool CaveDungeon::LoadTileSet(pugi::xml_node mapFile){
...
bool CaveDungeon::LoadLayer(pugi::xml_node& node, MapLayerC* layer)
{
    bool ret = true;
...
bool CaveDungeon::LoadAllLayers(pugi::xml_node mapNode) {
    ...
    {
        //Load the layer
        MapLayerC* mapLayer = new MapLayerC();
...
bool CaveDungeon::LoadProperties(pugi::xml_node& node, PropertiesC& properties)
{
    bool ret = false;

    for (pugi::xml_node propertieNode = node.child("properties").child("property"); propertieNode; propertieNode = propertieNode.next_sibling("property"))
    {
        PropertiesC::Property* p = new PropertiesC::Property();
...
PropertiesC::Property* PropertiesC::GetProperty(const char* name)
{
...
bool CaveDungeon::LoadColliders(pugi::xml_node& node) {

    bool ret = true;
    
    for (pugi::xml_node colLayerNode = node.child("objectgroup"); colLayerNode; colLayerNode = colLayerNode.next_sibling("objectgroup")) {
        ColTypesC ct = (ColTypesC)colLayerNode.child("properties").child("property").attribute("value").as_int();
...
void CaveDungeon::CreateColliders(ColDataC c) {
...
void CaveDungeon::DrawPath()
{
    iPoint point;

    // Draw visited
    ListItem<iPoint>* item = visited.start;
    if (app->cavedungeon->DrawPathing == true) {

        while (item)
        {
            point = item->data;
            TileSetC* tileset = GetTilesetFromTileId(119);
        ...
        for (uint i = 0; i < frontier.Count(); ++i)
        {
            ...
            TileSetC* tileset = GetTilesetFromTileId(118);
...
int CaveDungeon::MovementCost(int x, int y) const
{
    int ret = -1;
...
bool CaveDungeon::IsWalkable(int x, int y) const
{
    ...
    ListItem<MapLayerC*>* mapLayerItem;
    mapLayerItem = mapData.maplayers.start;
    MapLayerC* navigationLayer = mapLayerItem->data;
...
void CaveDungeon::ComputePath(int x, int y)
{
    path.Clear();
...
void CaveDungeon::PropagateDijkstra()
{
