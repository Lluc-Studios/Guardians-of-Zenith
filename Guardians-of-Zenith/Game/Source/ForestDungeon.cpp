...
bool ForestDungeon::Awake(pugi::xml_node& config)
{
...
void ForestDungeon::Draw()
{
    if(mapLoaded == false)
        return;

    ListItem<ImageLayerF*>* imageLayerItem;
    ...
    for (item = mapData.maplayers.start; item != NULL; item = item->next)
    {
        ...
        for (int y = 0; y < mapData.height; ++y)
        {
            for (int x = 0; x < mapData.width; ++x)
            {
                ...
                */
                TileSetF* tileset = (tileId > -1) ? GetTilesetFromTileId(tileId) : NULL;

                if (tileset != NULL)
                {
                    //map[i] = (tileId - tileset->firstgid) > 0 ? 0 : 1;
                    if (mapData.type == MapTypesF::MAPTYPEF_ISOMETRIC && tileId == 25) map[i] = 1;
                    else if (mapData.type == MapTypesF::MAPTYPEF_ORTHOGONAL && tileId == 119) map[i] = 1;
                    else map[i] = 0;
                }
                else {
...
iPoint ForestDungeon::MapToWorld(int x, int y) const
{
    iPoint ret;
...
iPoint ForestDungeon::WorldToMap(int x, int y)
{
    ...
    if (mapData.type == MAPTYPEF_ORTHOGONAL)
    {
        ret.x = x / mapData.tileWidth;
        ret.y = y / mapData.tileHeight;
    }
    else if (mapData.type == MAPTYPEF_ISOMETRIC)
    {
...
SDL_Rect TileSetF::GetTileRect(int gid) const
{
    SDL_Rect rect = { 0 };
...
TileSetF* ForestDungeon::GetTilesetFromTileId(int gid) const
{
    ListItem<TileSetF*>* item = mapData.tilesets.start;
    TileSetF* set = NULL;
...
bool ForestDungeon::LoadMap(pugi::xml_node mapFile)
{
    bool ret = true;
...
bool ForestDungeon::LoadTileSet(pugi::xml_node mapFile){
...
bool ForestDungeon::LoadLayer(pugi::xml_node& node, MapLayerF* layer)
{
    bool ret = true;
...
bool ForestDungeon::LoadAllLayers(pugi::xml_node mapNode) {
    ...
    {
        //Load the layer
        MapLayerF* mapLayer = new MapLayerF();
...
bool ForestDungeon::LoadProperties(pugi::xml_node& node, PropertiesF& properties)
{
    bool ret = false;

    for (pugi::xml_node propertieNode = node.child("properties").child("property"); propertieNode; propertieNode = propertieNode.next_sibling("property"))
    {
        PropertiesF::Property* p = new PropertiesF::Property();
...
PropertiesF::Property* PropertiesF::GetProperty(const char* name)
{
...
bool ForestDungeon::LoadColliders(pugi::xml_node& node) {

    bool ret = true;
    
    for (pugi::xml_node colLayerNode = node.child("objectgroup"); colLayerNode; colLayerNode = colLayerNode.next_sibling("objectgroup")) {
        ColTypesF ct = (ColTypesF)colLayerNode.child("properties").child("property").attribute("value").as_int();
...
void ForestDungeon::CreateColliders(ColDataF c) {
...
void ForestDungeon::DrawPath()
{
    iPoint point;

    // Draw visited
    ListItem<iPoint>* item = visited.start;
    if (app->forestdungeon->DrawPathing == true) {

        while (item)
        {
            point = item->data;
            TileSetF* tileset = GetTilesetFromTileId(119);
        ...
        for (uint i = 0; i < frontier.Count(); ++i)
        {
            ...
            TileSetF* tileset = GetTilesetFromTileId(118);
...
int ForestDungeon::MovementCost(int x, int y) const
{
    int ret = -1;
...
bool ForestDungeon::IsWalkable(int x, int y) const
{
    ...
    ListItem<MapLayerF*>* mapLayerItem;
    mapLayerItem = mapData.maplayers.start;
    MapLayerF* navigationLayer = mapLayerItem->data;
...
void ForestDungeon::ComputePath(int x, int y)
{
    path.Clear();
...
void ForestDungeon::PropagateDijkstra()
{
